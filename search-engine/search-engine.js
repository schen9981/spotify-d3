...
function main() {
	document.getElementById('fartist').on //
	updateArtist('Radiohead')
}

function enterKeyPressedOnTextbook(e) {
	if (e.key === 'Enter') {
...
function update_data() {
	...
	data_bind.enter()
		...
		content.exit()
			.transition()
				.duration(100)
				.style("opacity", "0").delay(200).remove()
}
...
function updateArtistAlbums(artistID) {
	s.getArtistAlbums(artistID, {album_type: "albums"}, function(err, albums) {
		if (err) { console.err(err); }
		else {
			data = [];
			var alreadyDisplayed = [];
			for (i = 0; i < albums.items.length; i++) {
				if (!alreadyDisplayed.includes(album.items[i].name)) {
					data[data.length] = {
						image: albums.items[i].images[0],
